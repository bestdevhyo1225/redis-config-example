...
import com.example.redisconfiguration.repository.MemberRedisServer1Repository
...
class MemberOldService(
    ...
    fun get(id: Long): FindMemberCacheResultDto {
        ...
        try {
            memberRedisServer1Repository.get(key = key, clazz = Member::class.java)
        ...
        runBlocking {
            launch(context = Dispatchers.IO) { setMemberCacheUsingCoroutine(key = key, value = value) }
        }
    ...
    fun getUsingPipeline(start: Int, count: Int): List<FindMemberCacheResultDto> {
        ...
        try {
            val cacheValues = memberRedisServer1Repository.getUsingPipeline(keys = keys, clazz = Member::class.java)
                .filterNotNull()
    ...
    suspend fun <T : Any> setMemberCacheUsingCoroutine(key: String, value: T) {
        memberRedisServer1Repository.set(key = key, value = value, expireTime = RedisExpireTime.MEMBER, timeUnit = TimeUnit.SECONDS)
    }
